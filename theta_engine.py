...
def _select_delta(snapshot: pd.DataFrame, side: str, target: float):
    candidates = []
    ...
    if not candidates:
        return None
    idx, _, g = min(candidates, key=lambda z: z[1])
    out = snapshot.loc[idx].copy()
    out["delta_calc"] = g["delta"]
    out["theta_hour_calc"] = g["theta_hour"]
    out["theta_efficiency"] = abs(g["theta_hour"]) / max(float(out["close"]), 0.05)
    return out

def _select_theta(snapshot: pd.DataFrame, side: str, min_delta: float = 0.18, max_delta: float = 0.50):
    candidates = []
    for idx, row in snapshot[snapshot["option_type"] == side].iterrows():
        dte = max((row["expiry"] - row["timestamp"].date()).days + 0.01, 0.01)
        g = greeks(float(row["spot"]), float(row["strike"]), float(row["iv"]), dte, side)
        if not g or float(row["close"]) <= 0:
            continue
        ad = abs(g["delta"])
        if min_delta <= ad <= max_delta:
            score = abs(g["theta_hour"]) / max(float(row["close"]), 0.05)
            candidates.append((idx, score, g))
    if not candidates:
        return _select_delta(snapshot, side, 0.30)
    idx, _, g = max(candidates, key=lambda z: z[1])
    out = snapshot.loc[idx].copy()
    out["delta_calc"] = g["delta"]
    out["theta_hour_calc"] = g["theta_hour"]
    out["theta_efficiency"] = abs(g["theta_hour"]) / max(float(out["close"]), 0.05)
...
def backtest_theta(df: pd.DataFrame, strategy: str = "Dynamic Theta Strangle", entry_time: str = "09:30", exit_time: str = "15:15", target_delta: float = 0.30, wing_distance: int = 4, lot_size: int = 65):
    x = _prepare(df)
    trades: list[Trade] = []
    if x.empty:
        return pd.DataFrame(), {}
    for session_date, day in x.groupby(x["timestamp"].dt.date, sort=True):
        entry_ts, exit_ts = _nearest_ts(day, entry_time), _nearest_ts(day, exit_time)
        ...
        if strategy == "ATM Straddle":
            ce, pe = _row(entry, "CE", atm), _row(entry, "PE", atm)
        elif strategy == "Delta 30 Strangle":
            ce, pe = _select_delta(entry, "CE", target_delta), _select_delta(entry, "PE", target_delta)
        else:
            ce, pe = _select_theta(entry, "CE"), _select_theta(entry, "PE")
        ...
        ce_out, pe_out = _price(day, exit_ts, "CE", ce_k), _price(day, exit_ts, "PE", pe_k)
...
def compare_strategies(df: pd.DataFrame, strategies: Iterable[str] | None = None, **kwargs):
    strategies = list(strategies or ["ATM Straddle", "Delta 30 Strangle", "Dynamic Theta Strangle", "Dynamic Theta Iron Condor"])
    rows, details = [], {}
